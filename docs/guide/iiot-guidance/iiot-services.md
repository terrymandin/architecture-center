...
A time series service will be required to provide a warm and cold store for the JSON-based industrial time series data. We recommend using [Time Series Insights (or TSI)](https://docs.microsoft.com/azure/time-series-insights/overview-what-is-tsi) as the time series service for the following reasons:

- Ability to ingest streaming data directly from [Azure IoT Hub](https://docs.microsoft.com/azure/iot-hub/about-iot-hub) or [Azure Event Hub](https://docs.microsoft.com/azure/event-hubs/event-hubs-about).
...
You can continue to use Time Series Insights with the old pricing model (S1, S2) for warm, unplanned data analysis. However, we highly encourage you to use the updated offering (PAYG) as it offers many new capabilities as well as Pay-As-You-Go pricing, cost savings, and flexibility to scale.
...
- The Azure function calls the OPC Twin microservice, which directs the OPC Publisher module to subscribe to the new tags.
  ...
  - If your solution involves third-party software, instead of OPC Publisher, configure the Azure Function to call an API running on the third-party software either directly or using an IoT Hub [Direct Method](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-direct-methods).
...
1. Import your data.
      ...
      1. Make sure to use the same [Time Series ID](https://docs.microsoft.com/azure/time-series-insights/how-to-select-tsid) as you do for your streaming data.
...
The IIoT analytics solution can also include [Office 365](https://docs.microsoft.com/office365/) services to automate tasks and send notifications. The following are a few examples:
...
The [Time Series Model](https://docs.microsoft.com/azure/time-series-insights/concepts-model-overview) in Azure Time Series Insights provides asset hierarchy capabilities. Through the use of *Instances*, *Types* and *Hierarchies*, you can store metadata about your industrial devices, as shown in the image below.
...
In the future, asset models will evolve to become [digital twins](https://docs.microsoft.com/azure/digital-twins/concepts-twins-graph), combining dynamic asset data (real-time telemetry), static data (3D models, metadata from Asset Management Systems), and graph-based relationships, allowing the digital twin to change in real-time along with the physical asset.

[Azure Digital Twins](https://docs.microsoft.com/azure/digital-twins/) is an Azure IoT service that provides the ability to:
